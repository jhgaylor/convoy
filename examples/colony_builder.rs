...
fn colony_logic(out: &mut Out) {
    ...
    // BUILDER strategy: stack up to three refineries before shipping, then send
    // big late convoys. Slow to start, dominant on throughput once it's rolling.
    if goods >= 40 && refineries < 3 && !refinery_building {
        let (px, py) = (spawner_x + 1 + refineries, spawner_y);
        out.push(OP_BUILD, 0, BLD_REFINERY as i32, ((px << 8) | py) as i32);
    } else if refineries >= 2 && goods >= 35 {
